...
<Tabs items={['Codex CLI', 'Qwen Code', 'OpenCode']}>

### Codex CLI

If you want to use OpenAI Codex as the execution backend, install and authenticate Codex CLI:
...
# Configure Pilot to use Codex CLI
...
See [Execution Backends](/concepts/execution-backends#codex-cli) for detailed configuration and feature comparison.
...
- [ ] Claude Code installed (`claude --version` outputs a version), or another configured execution backend is installed
- [ ] Execution backend authenticated (Claude subscription/API key, Codex login/API key, or provider-specific auth)
