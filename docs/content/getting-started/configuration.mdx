...
Pilot's runtime config is global for the Pilot installation. Use `host` to keep the existing local execution behavior, or `opensandbox` to run each task in a short-lived sandbox.

```yaml
runtime:
  provider: "host"                       # "host" or "opensandbox"
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `provider` | string | `"host"` | Runtime provider for all tasks: `host` or `opensandbox` |
| `opensandbox.endpoint` | string | — | OpenSandbox lifecycle API endpoint |
| `opensandbox.api_key` | string | — | API key sent as `OPEN-SANDBOX-API-KEY` |
| `opensandbox.cleanup.always_delete` | bool | `true` | Delete sandboxes after task completion |
| `opensandbox.cleanup.retain_on_failure` | bool | `false` | Keep failed sandboxes for debugging |
| `opensandbox.secrets` | map | `{}` | Operator-owned mapping from repo-declared secret names to environment variables |

Repos that run in OpenSandbox declare their execution requirements in `.pilot/environment.yaml`. The `sandbox` block is passed through to OpenSandbox; the `pilot` block describes workspace checkout, setup, verification, and required secrets.
...
<Callout type="info">
`runtime.provider` is not repo-authored. If an installation is configured for OpenSandbox, every eligible task uses OpenSandbox; the repo only describes the sandbox it needs.
</Callout>
...
| `decompose.enabled` | bool | `false` | Auto-decompose complex tasks into subtasks ([Epic Decomposition docs](/features/epic-decomposition)) |
...
[Read the hooks docs →](/features/hooks)
