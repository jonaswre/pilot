...
func (b *ClaudeCodeBackend) executeWithFromPR(ctx context.Context, opts ExecuteOptions, allowFromPR bool) (*BackendResult, error) {
	spec := b.buildCommandSpec(opts, allowFromPR)
...
func (b *ClaudeCodeBackend) buildCommandSpec(opts ExecuteOptions, allowFromPR bool) pilotruntime.CommandSpec {
	var args []string
	...
	env := map[string]string{"PILOT_EXECUTOR": "1"}
	// GH-2371: route the CC subprocess to the configured provider when set.
	if b.apiBaseURL != "" {
		env["ANTHROPIC_BASE_URL"] = b.apiBaseURL
	}
	if b.apiAuthToken != "" {
		env["ANTHROPIC_AUTH_TOKEN"] = b.apiAuthToken
	}
	if b.defaultModel != "" {
		env["ANTHROPIC_MODEL"] = b.defaultModel
	}
	...
	if b.config.MaxOutputTokens > 0 {
		env["CLAUDE_CODE_MAX_OUTPUT_TOKENS"] = fmt.Sprintf("%d", b.config.MaxOutputTokens)
	}
