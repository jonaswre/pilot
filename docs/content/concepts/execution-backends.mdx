...
- **Codex CLI**: OpenAI's coding agent CLI with `codex exec --json` automation
...
All backends execute the same Pilot pipeline and produce pull requests. The differences lie in model capabilities, configuration complexity, and feature parity.
...
|---------|------------|-----------|-----------|----------|
| **Stream JSON output** | Yes | Yes (`codex exec --json`) | Yes (v0.1.0+) | SSE (streaming) |
| **Session resume** | Yes (`--resume`) | Yes (`codex exec resume`) | Yes | No |
| **PR context (`--from-pr`)** | Yes | No | No | No |
| **Effort routing** | Yes | Yes (`model_reasoning_effort`) | No (silently skipped) | No |
| **Model routing** | Yes | Yes | Yes | Via config |
| **Permissions skip** | `--dangerously-skip-permissions` | `--dangerously-bypass-approvals-and-sandbox` | `--yolo` | N/A |
| **Verbose mode** | `--verbose` | JSONL events | Not supported | N/A |
| **Error retry (rate limit, API)** | Yes | Yes | Yes | No (raw HTTP) |
| **Session-not-found fallback** | Yes (`--from-pr`/`--resume` retry) | Yes (`resume` retry) | Yes (`--resume` retry) | No |
...
OpenAI's Codex CLI backend runs Pilot tasks through `codex exec --json`. It uses the same subprocess lifecycle as the other CLI backends: heartbeat timeout, watchdog timeout, stderr diagnostics, rate-limit/API error classification, and session-not-found fallback.

### Prerequisites

```bash
npm install -g @openai/codex
codex login  # or provide Codex-supported API key auth for automation
codex --version
```
...
| `extra_args` | []string | `[]` | Additional arguments appended to `codex exec` before the prompt |
| `use_session_resume` | bool | `false` | Use `codex exec resume <thread_id>` for context continuation |

### Important Notes

- **Automation mode**: Pilot invokes `codex exec --json --sandbox danger-full-access --dangerously-bypass-approvals-and-sandbox`.
- **Session resume**: Pilot captures the `thread_id` from `thread.started` and can use it for self-review when `use_session_resume` is enabled.
- **Effort routing**: Pilot maps `max` to Codex's `xhigh` reasoning effort and passes other supported effort values through `model_reasoning_effort`.
- **No PR context flag**: Codex CLI does not have a `--from-pr` equivalent, so Pilot ignores `FromPR` for this backend.

### Strengths

- ✅ OpenAI Codex execution with native JSONL automation
- ✅ Session resume for context reuse
- ✅ Model and reasoning-effort routing
- ✅ Rate limit, API, timeout, and session errors classified for retries

### Limitations

- ❌ No `--from-pr` equivalent
- ❌ Claude-specific `allowed_tools` / `mcp_config_path` settings do not apply

### Best For

- Teams using OpenAI Codex as their coding agent
- Environments standardizing on OpenAI authentication and models
- Users who want a local CLI backend without an HTTP server
...
| OpenAI Codex CLI execution | Codex CLI |
...
```yaml
executor:
  type: "codex-cli"  # Change from "claude-code" to "codex-cli"
  codex_cli:
    command: "codex"
    use_session_resume: true
```
...
**Codex CLI**: Requires `codex exec --json` support and `codex_cli.use_session_resume: true`

**Qwen Code**: Requires `qwen --version` to show v0.1.0+

### Rate limit errors

**Claude Code**, **Codex CLI**, and **Qwen Code** automatically retry with exponential backoff. **OpenCode** does not — you must handle retries at the Pilot orchestrator level.
