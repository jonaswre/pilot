...
Controls how Pilot runs execution backends (Claude Code, Codex CLI, Qwen Code, or OpenCode) to execute tasks.

```yaml
executor:
  type: "claude-code"                     # "claude-code", "codex-cli", "qwen-code", or "opencode"
  auto_create_pr: true
  ...
  codex_cli:
    ...
    extra_args: []                        # additional codex exec arguments
    use_session_resume: false             # reuse Codex threads for self-review
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `type` | string | `"claude-code"` | Backend type: `claude-code`, `codex-cli`, `qwen-code`, or `opencode` |
...
| `codex_cli.command` | string | `"codex"` | Path to the Codex CLI binary |
| `codex_cli.extra_args` | []string | `[]` | Additional `codex exec` arguments |
| `codex_cli.use_session_resume` | bool | `false` | Reuse Codex threads for self-review |
...
<Tabs items={['Claude Code', 'Codex CLI', 'OpenCode']}>
...
**Codex CLI backend fields**

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `codex_cli.command` | string | `"codex"` | Path to the Codex CLI binary |
| `codex_cli.extra_args` | []string | `[]` | Additional arguments appended to `codex exec` before the prompt |
| `codex_cli.use_session_resume` | bool | `false` | Continue prior Codex threads via `codex exec resume <thread_id>` |
